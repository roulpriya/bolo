...
function normalizeKey(keys) {
  ...
  return keys
    .map((key) => replacements[String(key).toUpperCase()] || key)
    .join("+");
}
...
export class BrowserPageComputer {
  ...
  async usablePage() {
    if (!this.page || this.page.isClosed?.()) {
      if (!this.manager) {
        throw new Error("The browser page has been closed.");
      }
  ...
  async drag(path) {
    ...
    await this.perform("Dragging in browser", async (page) => {
      ...
      for (const [x, y] of path.slice(1)) {
        // biome-ignore lint/performance/noAwaitInLoops: Pointer moves must remain ordered to preserve the drag path.
        await page.mouse.move(x, y);
      }
      await page.mouse.up();
    });
...
export class LocalBrowserManager {
  ...
  async newPage() {
    ...
    for (let attempt = 0; attempt < 2; attempt += 1) {
      // biome-ignore lint/performance/noAwaitInLoops: The second attempt depends on invalidating the first closed browser context.
